...
std::string LoadCodePageController::HandleRequestThrow(
...
userver::formats::json::Value LoadCodeController::HandleRequestJsonThrow(
    const userver::server::http::HttpRequest&,
    ...
    const std::string& code1, const std::string& code2) const
{
    ...
    long double pct = apotheosis::check_two_graphs(res+"0.cpp/output.dot", res+"1.cpp/output.dot");
    int ipct = static_cast<int>(pct*100);
    return "Codes identical for "+std::to_string(ipct)+"%";
}

userver::formats::json::Value ManyCodesCheck::HandleRequestJsonThrow(
    const userver::server::http::HttpRequest&,
